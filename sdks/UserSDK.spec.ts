...
import {
  userCreateWithDefaultName, userCreateWithName1, userCreateWithName2, userCreateWithName3,
...
describe('UserSDK', async () => {
  ...
  it('should create user with "guest" as _id and name if name is not passed', async () => {
    const timeBefore = Date.now();
    const createResponse = await userSDK.create(userCreateWithDefaultName);
    const readResponse = await userSDK.read();
    expect(createResponse._id).to.be.equal(DEFAULT_USER);
    expect(readResponse._id).to.be.equal(DEFAULT_USER);
  ...
  it('should fetch default "guest" if _id is not passed', async () => {
    const readResponse = await userSDK.read();
    expect(readResponse._id).to.be.equal(DEFAULT_USER);
    expect(readResponse.name).to.be.equal(DEFAULT_USER);
  });
  it('should create user with passed name as name and lowercase of name as _id', async () => {
    const createResponse = await userSDK.create(userCreateWithName1.data);
    const readResponse = await userSDK.read(userCreateWithName1._id);
    expect(createResponse._id).to.be.equal(userCreateWithName1._id);
    expect(readResponse.name).to.be.equal(userCreateWithName1.data.name);
  });
  it('should create user with passed name as name and whiteSpace removed name as _id', async () => {
    const createResponse = await userSDK.create(userCreateWithName2.data);
    const readResponse = await userSDK.read(userCreateWithName2._id);
    expect(createResponse._id).to.be.equal(userCreateWithName2._id);
    expect(readResponse.name).to.be.equal(userCreateWithName2.data.name);
  });
  it('should create user with passed name as name and whiteSpace removed and lowercase of name as _id', async () => {
    const createResponse = await userSDK.create(userCreateWithName3.data);
    const readResponse = await userSDK.read(userCreateWithName3._id);
    expect(createResponse._id).to.be.equal(userCreateWithName3._id);
    expect(readResponse.name).to.be.equal(userCreateWithName3.data.name);
  });
